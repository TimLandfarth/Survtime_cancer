...
kbl(coef, booktabs = T, digits = 3, position = "h", align = "rrrr") %>%
  kable_styling(position = "center", latex_options = "HOLD_position") %>%
    add_header_above(c(" " = 1,"BIC model" = 2, "AIC model" = 2))
```
In both cases, a higher albumin level is expected to lead to a lower risk of survival.
...
In the case of the AIC model, a good separation between the two groups can be recognized, while in the BIC model the separation is barely visible. In both cases, however, it must be explicitly warned that these can be over- and/or underoptimistic.

Lieber Herr Hornung, ich weiß leider einfach nicht, warum dies so ist. Mir ist dabei nur eingefallen, dass:
- die imputierten Werte zu optimistisch (oder pessimistisch) gezogen wurden (optimistisch: zu nahe an kleinem Risiko; pessimistisch: Normalverteilungsannahme unter pamm bricht)
- Modellselektion: AIC: bessere Trennung aufgrund von mehr Kovariablen (also vermeintlich genauer); BIC: schlechtere Trennung aufgrund von weniger Kovariablen (also vermeintlich ungenauer)
...
### Model
For validation, a 5-fold simple cross-validation was repeated ten times with 20-fold multiple imputation. Outside the cross-validation, the original data set was divided into test and training data, while within the cross-validation the test and training data were imputed. Variables were selected on the imputed training data using AIC and BIC (taking into account formula $\frac{M}{1-f}$ described above). The Concordance Index (discrimination) and the integrated Brier Score (calibration) were calculated on each imputed test data set of the twenty-fold imputation using the partial test data and the mean value of the Concordance Index and Brier Score was estimated over the twenty imputations.
...
perf <- data.frame(AIC = as.character(c(paste(round(mean(res$cindex_model.aic),3), " (", round(sd(res$cindex_model.aic),3), ")", sep = ""),
                              paste(round(mean(res$brier_model.aic),3), " (", round(sd(res$brier_model.aic),3), ")", sep = ""))),
           BIC = as.character(c(paste(round(mean(res$cindex_model.bic),3), " (", round(sd(res$cindex_model.bic),3), ")", sep = ""),
                              paste(round(mean(res$brier_model.bic),3), " (", round(sd(res$brier_model.bic),3), ")", sep = ""))))
