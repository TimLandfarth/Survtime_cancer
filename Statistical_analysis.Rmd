...
## Descriptive
The intention was to statistically investigate the influence of findings from the hemogram on the progression of tumor diseases within the new therapy using *describe new therapy in more detail here*. For this purpose, a survival time analysis was carried out using the Cox proportional hazard model.
...
## Model
To calculate the Cox proportional hazard model, the selection of the formula was calculated using the AIC and BIC, corrected for the imputed data (more observations and proportion of imputed values), using forward selection, resulting in the following values: 
...
The linear predictor for the AIC and BIC model was calculated for all observations, ordered (from minimum to maximum). Then, the first ordered observation and the remaining observations were divided into two groups and the logrank statistic was estimated. The two groups were then created based on the first two ordered observations, with the rest of the procedure as above. 
